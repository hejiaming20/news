  ...
  <div>
    ...
    <el-row :gutter="5">
      <el-col
        ...
        <div
          class="layout-small-title-block">
          <span>1#磨床状态<el-switch
            v-model="value1"
            :active-value="1"
            :inactive-value="0"
      ...
      <el-col
        ...
        :lg="12"
        class="layout-default-margin">
        <div class="layout-small-title-block"><span>2#磨床状态
          <el-switch
            v-model="value2"
            :active-value="1"
            :inactive-value="0"
  ...
  methods: {
    ...
    moChange1() {
      if (this.value1 == 0) {
        ...
        this.updataMoState(1, this.value1)
      }
    },
    moChange2() {
      if (this.value2 == 0) {
        ...
        this.updataMoState(2, this.value2)
      }
    },
    // 查看磨床状态
    async findmoState1() {
      let res = await post('rollMachine/findstate', { indocno: 1 })
      if (res.status == 2000) {
        this.value1 = res.data
      }
    },
    async findmoState2() {
      let res = await post('rollMachine/findstate', { indocno: 2 })
      if (res.status == 2000) {
    ...
    /* handleremove_up(row, num, index) {
      if (index != 0) {
        ...
        //   this.roll_num.push({ ilevel: item.ilevel, roll_no: item.roll_no })
        // })
        this.roll_2 = this.tableData1[index - 1].roll_no
        this.ilevel_2 = this.tableData1[index - 1].ilevel
        post('rollInformation/changelevel', {
          a_roll: this.roll_1,
          a_level: this.ilevel_1,
          b_roll: this.roll_2,
